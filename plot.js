...
function plotData(map) {
    ...
    filteredData = data.filter((data) => { return data.year == "1980" });
...
function plotPopulation(country, toolChart) {
    ...
    axesLimits.yMax = axesLimits.yMax / 1000000;
    axesLimits.yMin = axesLimits.yMin / 1000000;
    ...
    makePopLabels(toolChart, small_msm, "Population Over Time For " + country, "Year", "Population (in Millions)");
}

// make title and axes labels
function makePopLabels(svgContainer, msm, title, x, y) {
    svgContainer.append('text')
        .attr('x', (msm.width - 2 * msm.marginAll) / 2 - 90)
        .attr('y', msm.marginAll / 2 + 10)
        .style('font-size', '10pt')
    ...
    svgContainer.append('text')
        .attr('transform', 'translate( -25,' + (msm.height / 2 + 50) + ') rotate(-90)')
        .style('font-size', '10pt')
        .text(y);
}
